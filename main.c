...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/26 13:01:32 by ele-cren          #+#    #+#             */
/*   Updated: 2017/02/07 10:51:36 by ele-cren         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_args(char **av, t_size *size, t_args ag)
{
	...
	while (av[i])
	{
		...
		if (ft_strcmp(av[i], ag.arg1) == 0)
			size->deep = (ft_atoi(av[i + 1]) > 0) ? ft_atoi(av[i + 1]) : 1;
		else if (ft_strcmp(av[i], ag.arg2) == 0)
			size->scale = (ft_atoi(av[i + 1]) > 0) ? ft_atoi(av[i + 1]) : 20;
		else if (ft_strcmp(av[i], ag.arg3) == 0)
			size->scale2 = (ft_atoi(av[i + 1]) > 0) ? ft_atoi(av[i + 1]) : 5;
		else
			ft_putendl("Usage : ./fdf map_file [-d] [-s] [-b]");
		i += 2;
	}
}
